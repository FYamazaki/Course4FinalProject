...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        res = scipy.optimize.minimize(self.objective, p0, jac=self.objectie_grad, bounds=bounds, method='L-BFGS-B')
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        thetas = self.thetaf(a, b, c, d, s_points)
        t_points = thetas
        x_points = scipy.integrate.cumtrapz(cos(thetas), s_points)
        y_points = scipy.integrate.cumtrapz(sin(thetas), s_points) 
